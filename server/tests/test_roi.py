...
class TestROINormalization(unittest.TestCase):
    ...
    def test_normalize_roi_8_elements(self):
        """Test ROI normalization with 8 elements (legacy format - now returns 10 fields)."""
        roi = (1, 2, (100, 100, 200, 200), 305, 3000, 0.9, "mobilenet", 0)
        normalized = self.normalize_roi(roi)
        
        self.assertEqual(len(normalized), 11)  # Now returns 11-field format
        ...
        self.assertEqual(normalized[8], 1)  # device_location (default)
        self.assertIsNone(normalized[9])  # expected_text (default)
    
    def test_normalize_roi_7_elements(self):
        """Test ROI normalization with 7 elements (legacy format - now returns 10 fields)."""
        ...
        self.assertEqual(normalized[7], 0)  # rotation (default)
        self.assertEqual(normalized[8], 1)  # device_location (default)
        self.assertIsNone(normalized[9])  # expected_text (default)
    
    def test_normalize_roi_6_elements(self):
        """Test ROI normalization with 6 elements (legacy format - now returns 10 fields)."""
        roi = (3, 2, (0, 0, 100, 100), 305, 3000, 0.95)
        normalized = self.normalize_roi(roi)
        
        self.assertEqual(len(normalized), 11)  # Now returns 11-field format
        self.assertEqual(normalized[5], 0.95)
        self.assertEqual(normalized[6], "mobilenet")  # Default method added for type 2
        self.assertEqual(normalized[7], 0)  # rotation (default)
        self.assertEqual(normalized[8], 1)  # device_location (default)
        self.assertIsNone(normalized[9])  # expected_text (default)
    ...
    def test_normalize_roi_object_format(self):
        """Test ROI normalization with object (dict) format."""
        roi = {
            "idx": 5,
            "type": 2,
        ...
        self.assertEqual(normalized[0], 5)  # idx
        self.assertEqual(normalized[1], 2)  # type
        self.assertEqual(normalized[2], (100, 200, 300, 400))  # coords as tuple
        self.assertEqual(normalized[3], 310)  # focus
        self.assertEqual(normalized[4], 1500)  # exposure
        self.assertEqual(normalized[5], 0.95)  # ai_threshold
        self.assertEqual(normalized[6], "mobilenet")  # feature_method
        self.assertEqual(normalized[7], 90)  # rotation
        self.assertEqual(normalized[8], 3)  # device_location
        self.assertEqual(normalized[9], "TEST")  # expected_text
        self.assertTrue(normalized[10])  # is_device_barcode
