...
try:
    ...
    class TestVisualAOILogic(unittest.TestCase):
        ...
        def test_roi_normalization_barcode(self):
            ...
            norm = normalize_roi(roi)
            self.assertEqual(len(norm), 10)  # Now returns 10-field format
            ...
            self.assertEqual(norm[7], 0)  # rotation (default)
            self.assertEqual(norm[8], 1)  # device_location (default)
            self.assertIsNone(norm[9])  # expected_text (default)
