"""AI processing module using PyTorch.
Uses PyTorch MobileNetV2 for better RTX 5080 compatibility.
Falls back to OpenCV features if PyTorch is unavailable.
"""
...
# Enable PTX JIT compilation for RTX 5080 (compute capability 12.0)
# This allows PyTorch to run on sm_120 GPUs using JIT-compiled PTX code
...
transforms = None
mobilenet_v2 = None
model_device = 'cpu'  # Default to CPU
gpu_available = False
...
try:
    ...
    print("PyTorch not available: No module named 'torch'")
...
# PyTorch 2.5.1 stable has better compatibility than 2.9 nightly
...
def initialize_mobilenet_model():
    ...
    try:
        ...
        with torch.no_grad():
            test_device = device
            try:
                dummy_input = torch.randn(1, 3, 224, 224).to(test_device)
                output = mobilenet_model(dummy_input)
                print(f"✓ Model test successful on {test_device} - output shape: {output.shape}")
            except Exception as test_error:
                if "CUDA" in str(test_error) and str(test_device) != 'cpu':
                    print(f"⚠ GPU test failed: {test_error}")
                    print("  Falling back to CPU for model...")
                    # Move model to CPU and retest
                    cpu_device = torch.device('cpu')
                    mobilenet_model = mobilenet_model.to(cpu_device)
                    model_device = str(cpu_device)
                    # Update global device variable so feature extraction uses correct device
                    globals()['device'] = cpu_device
                    
                    dummy_input = torch.randn(1, 3, 224, 224).to(cpu_device)
                    output = mobilenet_model(dummy_input)
                    print(f"✓ Model test successful on CPU - output shape: {output.shape}")
                else:
                    raise test_error
...
def ai_extract_features_from_array(img_array):
    ...
    try:
        ...
        print("Permanently disabling PyTorch model and falling back to OpenCV for all future extractions...")
        # Disable PyTorch globally to ensure consistent feature dimensions
        mobilenet_model = None
        return opencv_extract_features_from_array(img_array)
...
def cosine_similarity(a, b):
    ...
    Handles feature vectors of different dimensions by padding the smaller one.
    """
    # Handle dimension mismatch by padding with zeros
    if len(a) != len(b):
        print(f"Warning: Feature dimension mismatch ({len(a)} vs {len(b)}). Padding to match.")
